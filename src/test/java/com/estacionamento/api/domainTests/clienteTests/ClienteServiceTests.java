...
public class ClienteServiceTests {
    ...
    public void testCadastrarCliente() {
        var criarCliente = UtilsMock.cadastrarClienteJSON();
        when(clienteRepository.save(any(Cliente.class))).thenReturn(cliente);

        Cliente result = clienteService.cadastrarCliente(criarCliente);
    ...
    public void testAtualizarCliente() {
        var atualizarCliente = UtilsMock.atualizarClienteJSON();
        when(clienteRepository.findById(anyLong())).thenReturn(Optional.of(cliente));
        when(clienteRepository.save(any(Cliente.class))).thenReturn(cliente);

        Cliente result = clienteService.atualizarCliente(1L, atualizarCliente);
