...
public class PagamentoServiceTest {
    ...
    public void testPagamentoTicket_RealizarPagamento() {
        ...
        when(pagamentoRepository.save(any(Pagamento.class))).thenReturn(new Pagamento(pagamentoTicketDto, ticket));
    ...
    public void testPagamentoPlanoMensal_RenovandoPlanoMensal() {
        when(clienteRepository.findById(anyLong())).thenReturn(Optional.of(UtilsMock.clienteMock_Cenario1()));
        when(clienteRepository.save(any(Cliente.class))).thenReturn(UtilsMock.clienteMock_Cenario1());
        double valor = TipoPlano.PREMIUM.getValorPlano();
        when(pagamentoRepository.save(any(Pagamento.class))).thenReturn(new Pagamento(pagamentoPlanoMensal, UtilsMock.clienteMock_Cenario1(), BigDecimal.valueOf(valor)));

        cliente.setTipoPlano(TipoPlano.PREMIUM);
        cliente.setPlanoMensalAtivo(true);
        LocalDateTime validade = UtilsMock.clienteMock_Cenario1().getValidadePlanoMensal();
    ...
    public void testPagamentoPlanoMensal_UpgradePlanoMensal() {
        when(clienteRepository.findById(anyLong())).thenReturn(Optional.of(UtilsMock.clienteMock_Cenario2()));
        when(clienteRepository.save(any(Cliente.class))).thenReturn(UtilsMock.clienteMock_Cenario2());
        long diasRestantes = ChronoUnit.DAYS.between(LocalDateTime.now(), UtilsMock.clienteMock_Cenario2().getValidadePlanoMensal());
        double valor = TipoPlano.calcularMigracaoDePlano(TipoPlano.BASIC, TipoPlano.PREMIUM, diasRestantes);
        when(pagamentoRepository.save(any(Pagamento.class))).thenReturn(new Pagamento(pagamentoPlanoMensal, UtilsMock.clienteMock_Cenario2(), BigDecimal.valueOf(valor)));

        cliente.setTipoPlano(TipoPlano.PREMIUM);
        cliente.setPlanoMensalAtivo(true);
        LocalDateTime validade = UtilsMock.clienteMock_Cenario2().getValidadePlanoMensal();
        cliente.setValidadePlanoMensal(validade.plusDays(30));
        Pagamento pagamento = pagamentoService.pagamentoPlanoMensal(pagamentoPlanoMensal);
